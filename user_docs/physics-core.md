...
## Model dimensions
Whilst it is most often used to model a single point, UCLCHEM is a 1D model. For most models, you provide the size of some cloud of gas (`rout`) and the number of positions along a 1D cut through this cloud you'd like to model (`points`). UCLCHEM then divides the cloud up into the required number of equally sized segments and then models a point at the inner edge of each segment.

![Cloud points illustration](../_static/cloud_points.png)
