...
#database_reaction_file : data/databases/kida.uva.2014.dat
...
# Define whether you want to extrapolate gas phase reactions beyond their measured range (recommend with UMIST22):
extrapolate: True
```
...
**extrapolate** turns on and off the extrapolation of rates beyond their measured temperature range. Since many rates now have a narrow measured range, sometimes of only a few 100 Kelvin, it is recommended to turn this on; If you turn it off, be aware of rates of 0.0 outside of the measured range.
...
The third reactant can be used as a keyword to tell MakeRates what kind of reaction is occuring. In the absence of a keyword, MakeRates and UCLCHEM will treat any reaction as a gas-phase two body reaction. Two keywords the user may wish to use are ER and LH for Eley-Rideal and Langmuir-Hinshelwood reactions respectively (see [Grain Chemistry](chem-grain.md)). If a keyword is not added, UCLCHEM will assume a reaction between two surface reactions follows a Kooji-Arrhenius equation.
