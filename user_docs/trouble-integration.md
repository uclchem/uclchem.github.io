...
One good method to check the validity of your solution is to use the element conservation functions in the {py:func}`uclchem.analysis.check_element_conservation` function. The integrator typically fails to conserve elemental abundances when the integration has accumulated too large an error. Thus, checking for conservation can reassure you that the integration was successful even if the integrator struggled.
